...
import { createKategori, deleteKategoriById, findAllKategori, updateKategori } from "../../service/KategoriService";
    ...
    const saveKategori = async () => {
        try {
            ...
            if (kategori.nama.trim()) {
                ...
                await load(); // ⬅️ selalu panggil ulang data dari server
                setInsertMode(false);
                setKategoriDialog(false);
                setKategori(emptyKategori);
                setSubmited(false);
                        ...
                        <Dialog visible={deleteKategoriDialog}
                            ...
                            <div className="confirmation-content">
                                <i className="pi pi-exclamation-triangle p-mr-3"
                                    style={{ fontSize: "2rem" }}
                                ></i>
                                {kategori && <span class="ml-1">Apakah anda yakin akan menghapus kategori <b>{kategori.nama}</b>?</span>}
                            </div>
                        </Dialog>
                    </div>
